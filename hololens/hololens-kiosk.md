...
|다중 앱 키오스크 |설정됨 |사용 <sup> 2</sup> |사용 가능 <sup> 2</sup> |사용 가능 <sup> 2</sup> |사용 가능한 <sup> 2, 3</sup>  |사용 <sup> 1</sup> |
...
MDM 환경에서는 그룹을 사용 하 여 장치 구성 및 사용자 액세스를 관리 합니다. 

키오스크 구성 프로필에는 **사용자 로그온 유형** 설정이 포함 되어 있습니다. **사용자 로그온 유형은** 추가 하는 앱을 사용할 수 있는 사용자 (또는 사용자가 포함 된 그룹)를 식별 합니다. 사용자가 구성 프로필에 포함 되지 않은 계정을 사용 하 여 로그인 하는 경우 해당 사용자는 키오스크에서 앱을 사용할 수 없습니다.  

> [!NOTE]  
> 단일 앱 키오스크의 **사용자 로그온 유형은** 단일 사용자 계정을 지정 합니다. 키오스크이 실행 되는 사용자 컨텍스트입니다. 다중 앱 키오스크의 **사용자 로그온 유형에** 는 키오스크를 사용할 수 있는 하나 이상의 사용자 계정 또는 그룹이 지정 될 수 있습니다.

키오스크 구성을 장치에 배포 하려면 먼저 디바이스 또는 장치에 로그인 할 수 있는 사용자를 포함 하는 그룹에 키오스크 구성 프로필을 *할당* 해야 합니다. 이 설정은 다음과 같은 동작을 생성 합니다.

- 장치가 할당 된 그룹의 구성원 인 경우 사용자가 장치에 처음 로그인 할 때 키오스크 구성이 장치에 배포 됩니다.  
- 장치가 할당 된 그룹의 구성원이 아니지만 해당 그룹의 구성원 인 사용자가 로그인 하면 해당 시간에 키오스크 구성이 디바이스에 배포 됩니다.
...
> [!NOTE]  
> 다음 예제에서는 다중 앱 키오스크에 대해 설명 합니다. 단일 앱 키오스크도 비슷한 방식으로 작동 하지만, 키오스크 환경을 제공 하는 사용자 계정은 한 명만 가능 합니다.

**예제 1**

장치 및 사용자 둘 다에 단일 그룹 (그룹 1)을 사용 합니다. 하나의 장치 및 사용자 A, B, C는이 그룹의 구성원입니다. 아래와 같이 키오스크 구성 프로필을 구성 합니다.  

- **사용자 로그온 유형**: 그룹 1
- **배정 된 그룹**: 그룹 1

장치에 먼저 로그인 하는 사용자와 관계 없이 (기본 제공 경험 또는 OOBE를 통해) 키오스크 구성은 장치에 배포 됩니다. 사용자 A, B, C가 모든 장치에 로그인 하 여 키오스크 환경을 활용할 수 있습니다.

**예제 2**

장치를 서로 다른 키오스크 환경을 필요로 하는 두 가지 다른 공급 업체에 게 제공 합니다. 두 공급 업체 모두 사용자를 보유 하 고 있으며 모든 사용자가 자신의 공급 업체와 다른 공급 업체 로부터 키오스크에 액세스할 수 있도록 하려는 경우 다음과 같이 그룹을 구성 합니다.
...
다음 설정을 사용 하는 두 개의 키오스크 구성 프로필을 만듭니다.
...
이러한 구성은 다음과 같은 결과를 생성 합니다.

- 사용자가 장치 1 또는 장치 2에 로그인 하면 Intune에서 키오스크 프로필 1을 해당 장치에 배포 합니다.
- 사용자가 장치 3 또는 장치 4에 로그인 하면 Intune에서 키오스크 프로필 2를 해당 장치에 배포 합니다.
- 사용자 A와 사용자 B는 네 가지 장치 중 하나에 로그인 할 수 있습니다. 장치 1 또는 장치 2에 로그인 하는 경우 공급 업체 1 키오스크 환경이 표시 됩니다. 장치 3 또는 장치 4에 로그인 하는 경우 공급 업체 2 키오스크 환경이 표시 됩니다.

#### 프로필 충돌

두 개 이상의 키오스크 구성 프로필이 동일한 장치를 대상으로 하는 경우 충돌 합니다. Intune 관리 장치의 경우 Intune은 충돌 하는 프로필을 적용 하지 않습니다.
...
|자동으로 배포      | 아니오            | 아니오                   | 예  |
|배포 속도            | 가장 빠른       | 빠름                 | 슬로우 |
|배율에 배치 | 권장 되지 않음    | 권장 되지 않음        | 권장 사항 |
