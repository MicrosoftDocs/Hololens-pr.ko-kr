...
다른 Windows 장치와 마찬가지로 HoloLens는 항상 사용자 컨텍스트에서 작동 합니다. 항상 사용자 id가 있습니다. HoloLens는 다른 Windows 장치에서 수행 하는 것과 거의 동일한 방식으로 id를 처리 합니다. 이 문서는 HoloLens에 대 한 id에 대 한 심층 참조 자료로, 다른 Windows 장치와 HoloLens 어떻게 다른 지에 대해 집중적으로 설명 합니다.

HoloLens는 여러 종류의 사용자 id를 지원 합니다. 하나 이상의 사용자 계정을 사용 하 여 로그인 할 수 있습니다. HoloLens의 id 유형 및 인증 옵션에 대 한 개요는 다음과 같습니다.

| ID 형식 | 장치당 계정 | 인증 옵션 |
| --- | --- | --- |
| [Azure Active Directory](/azure/active-directory/)<sup>1</sup>  | 64 | <ul><li>Azure 웹 자격 증명 공급자</li><li>Azure Authenticator 앱</li><li>생체 인식 (iri) &ndash; HoloLens 2<sup>2</sup> 만 해당 </li><li>FIDO2 보안 키</li><li>&ndash;HoloLens에 대 한 고정 옵션 (첫 번째 gen), HoloLens 2에 필요</li><li>암호</li></ul> |
| [MSA(Microsoft 계정)](/windows/security/identity-protection/access-control/microsoft-accounts) | 1 | <ul><li>생체 인식 (iri) &ndash; HoloLens 2만</li><li>&ndash;HoloLens에 대 한 고정 옵션 (첫 번째 gen), HoloLens 2에 필요</li><li>암호</li></ul> |
...
> [!NOTE]
> 2-HoloLens 2 장치에서 최대 64 개의 Azure AD 계정을 지원할 수 있지만 이러한 계정 중 31 개만 iri 인증에 등록할 수 있습니다. 이는 [Windows Hello for Business에 대 한 다른 생체 인식 인증 옵션과](/windows/security/identity-protection/hello-for-business/hello-faq#how-many-users-can-enroll-for-windows-hello-for-business-on-a-single-windows-10-computer)맞춰져 있습니다.

> [!IMPORTANT]
> 3-로컬 계정은 [OOBE 중에 프로 비전 패키지를 통해](hololens-provisioning.md#apply-a-provisioning-package-to-hololens-during-setup)장치에만 설정할 수 있으며 나중에 설정 앱에서 추가할 수 없습니다. 이미 설정 된 장치에서 로컬 계정을 사용 하려는 경우 [장치를 경감 하기 위해 하거나 다시 설정 해야 합니다.](hololens-recovery.md)

## <a name="setting-up-users"></a>사용자 설정

HoloLens에서 새 사용자를 설정 하는 방법에는 두 가지가 있습니다. 가장 일반적인 방법은 HoloLens OOBE (기본 제공 환경)를 실행 하는 것입니다. Azure Active Directory 사용 하는 경우 다른 사용자는 Azure AD 자격 증명을 사용 하 여 OOBE 후 [에 로그인 할 수 있습니다](#setting-up-multi-user-support-azure-ad-only) . OOBE를 실행 하는 동안 처음에 MSA 또는 로컬 계정으로 설정 된 HoloLens 장치는 여러 사용자를 지원 하지 않습니다. [HoloLens 설정 (첫 번째 gen)](hololens1-start.md) 또는 [HoloLens 2](hololens2-start.md)을 참조 하세요.
...
HoloLens는 동일한 Azure AD 테 넌 트의 여러 사용자를 지원 합니다. 이 기능을 사용 하려면 장치를 설정 하기 위해 조직에 속해 있는 계정을 사용 해야 합니다. 이후에는 동일한 테 넌 트의 다른 사용자가 로그인 화면에서 또는 시작 패널에서 사용자 타일을 눌러 장치에 로그인 할 수 있습니다. 한 번에 한 명의 사용자만 로그인 할 수 있습니다. 사용자가 로그인 하면 이전 사용자를 로그 아웃 HoloLens. 

>[!IMPORTANT]
> 장치의 첫 번째 사용자는 Azure AD 조인의 경우를 제외 하 고 장치 소유자에 게는 장치 소유자 [에 대 한 자세한](security-adminless-os.md#device-owner)정보를 알려 주는 것으로 간주 됩니다.
...
HoloLens 2은 iri 인증을 지원 합니다. iri는 Windows Hello 기술을 기반으로 하며 Azure Active Directory 및 Microsoft 계정 모두에서 사용 하도록 지원 됩니다. iri는 다른 Windows Hello 기술과 동일한 방식으로 구현 되며 [1/10만의 보안 생체 인식 보안](/windows/security/identity-protection/hello-for-business/hello-biometrics-in-enterprise#has-microsoft-set-any-device-requirements-for-windows-hello)을 달성 합니다.
...
### <a name="where-is-iris-biometric-information-stored"></a>조리개 생체 인식 정보는 어디에 저장 되나요?

iri 생체 인식 정보는 [Windows Hello 사양](/windows/security/identity-protection/hello-for-business/hello-biometrics-in-enterprise#where-is-windows-hello-data-stored)에 따라 각 HoloLens에 로컬로 저장 됩니다. 공유 되지 않으며 두 가지 암호화 계층으로 보호 됩니다. 관리자에 게는 HoloLens에 대 한 관리자 계정이 없으므로 다른 사용자도 액세스할 수 없습니다.

### <a name="do-i-have-to-use-iris-authentication"></a>Iri 인증을 사용 해야 하나요?
아니요, 설치 하는 동안이 단계를 건너뛸 수 있습니다. 

![설정 Iri](./images/setup-iris.png)

HoloLens 2는 FIDO2 보안 키를 비롯 한 다양 한 인증 옵션을 제공 합니다.

### <a name="can-iris-information-be-removed-from-the-hololens"></a>HoloLens에서 Iri 정보를 제거할 수 있나요?
