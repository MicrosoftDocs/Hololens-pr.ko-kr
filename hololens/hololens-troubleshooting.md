...
일부 고객은 파일을 업로드하거나 다운로드하려고 할 때 작업이 중단된 다음 시간 부족하거나 완료되지 않을 수 있음을 발견했습니다. 이는 알려진['파일 잠금' 문제와는](#downloading-locked-files-doesnt-error) 별개입니다. 이는 Holographic 버전 2004, 20H2 및 21H1 시장 내 빌드에 Windows 영향을 미칩니다. 이 문제는 장치 포털 특정 요청을 처리하는 버그로 인해 발생했으며, https(기본값)를 사용할 때 가장 일관되게 적중됩니다.
