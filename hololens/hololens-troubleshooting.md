...
Remote Assist 최신 빌드로 업데이트할 수 없는 경우 다음 해결 방법을 시도해 보세요.
...
- 동일한 주요 빌드에서 대규모 업데이트를 사용하도록 디바이스 업데이트(예: Windows Holographic 버전 2004에서 Windows Holographic 버전 20H2로 업데이트)
...
일부 고객은 Microsoft Edge 시작에 실패하는 문제를 보고했습니다. 이러한 고객의 경우 다시 부팅을 통해 문제가 지속되며 Windows 또는 애플리케이션 업데이트로 해결되지 않습니다. 이 문제가 발생하고 [Windows](hololens-updates.md#manually-check-for-updates)최신인지 확인한 경우 [피드백 허브 앱에서](hololens-feedback.md) 다음 범주 및 하위 범주인 설치 및 업데이트 > Windows 업데이트 다운로드, 설치 및 구성을 사용하여 버그를 제출하세요.
...
그런 다음 사용자는 https://(IP 주소)가 아니라 http:// 이동해야 하며 파일 업로드 및 다운로드와 같은 기능이 작동합니다.
...
- 디바이스 리플래시
    ...
    1. 연결되지 않은 상태에서 전원을 완전히 켜서 수동으로 HoloLens 2 [플래시 모드로](hololens-recovery.md) 전환합니다. 그런 다음, Volume up 누를 때 전원 단추를 누릅니다.
    
    1. PC에 커넥트 고급 복구 도우미를 엽니다.
