...
description: 연결 엔드포인트를 관리하고 구성하는 동안 Wi-Fi 네트워크를 통해 HoloLens를 설정하는 방법을 알아봅니다.
...
ms.locfileid: "115662945"
---
# <a name="manage-connection-endpoints-for-hololens"></a>HoloLens에 대한 연결 엔드포인트 관리

일부 HoloLens 구성 요소, 앱 및 관련 서비스는 Microsoft 네트워크 엔드포인트로 데이터를 전송합니다. 이 문서에서는 이러한 구성 요소가 작동하기 위해 네트워크 구성(예: 프록시 또는 방화벽)에서 허용해야 하는 다양한 엔드포인트 및 URL을 나열합니다.    

## <a name="near-offline-setup"></a>니어 오프라인 설정

HoloLens는 네트워크 환경 제한이 있는 고객을 위해 제한된 오프라인 환경 집합을 지원합니다. 그러나 HoloLens는 초기 디바이스 설정을 통과하는 네트워크 연결이 필요하고 다음 URL을 사용하도록 설정해야 합니다.
...
위 목록 외에 HoloLens 기능을 최대한 활용하려면 네트워크 구성에서 다음 엔드포인트를 사용하도록 설정해야 합니다.
...
| Microsoft 전달 링크 리디렉션 서비스(FWLink) | go.microsoft.com                                                    |
...
> [!NOTE]
> D365 Remote Assist를 배포하는 경우 [Office 365 URL 및 IP 주소 범위](/office365/enterprise/urls-and-ip-address-ranges#skype-for-business-online-and-microsoft-teams)에서 SharePoint Online 및 비즈니스용 OneDrive에 대해 나열된 엔드포인트를 활성화해야 합니다.

- [조직에서 Windows 진단 데이터 구성](/windows/privacy/configure-windows-diagnostic-data-in-your-organization)
- [Windows 10 Enterprise, 버전 1903용 연결 엔드포인트 관리](/windows/privacy/manage-windows-1903-endpoints)
- [Windows 10 운영 체제 구성 요소에서 Microsoft 서비스로의 연결 관리](/windows/privacy/manage-connections-from-windows-operating-system-components-to-microsoft-services)
- [Microsoft Intune MDM 서버를 사용하여 Windows 10 운영 체제 구성 요소에서 Microsoft 서비스로의 연결 관리](/windows/privacy/manage-connections-from-windows-operating-system-components-to-microsoft-services-using-mdm)
- [Intune 네트워크 구성 요구 사항 및 대역폭](/intune/fundamentals/network-bandwidth-use#network-communication-requirements)
- [Microsoft Intune에 대한 네트워크 엔드포인트](/intune/fundamentals/intune-endpoints)
- [Office 365 URL 및 IP 주소 범위](/office365/enterprise/urls-and-ip-address-ranges)
- [Azure AD Connect에 대한 필수 구성 요소](/azure/active-directory/hybrid/how-to-connect-install-prerequisites)


## <a name="hololens-limitations"></a>HoloLens 제한 사항

HoloLens를 설정한 후에는 Wi-Fi에 연결하지 않고 사용할 수 있지만 HoloLens를 오프라인으로 사용할 때 인터넷 연결을 사용하는 앱의 기능이 제한됩니다.
