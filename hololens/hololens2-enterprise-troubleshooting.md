...
1. Wi-Fi 프로필에 올바른 설정이 있는지 확인합니다.
    - EAP 종류를 올바르게 구성합니다. 일반적인 EAP 종류는 EAP-TLS(13), EAP-TTLS(21) 및 PEAP(25)입니다.
    - Wi-Fi SSID 이름을 확인하고, 16진수 문자열과 일치하는지 확인합니다.
    - EAP-TLS의 경우 서버의 신뢰할 수 있는 루트 CA 인증서의 SHA-1 해시가 TrustedRootCA에 포함되어 있는지 확인합니다. Windows PC에서 "certutil.exe -dump cert_file_name" 명령은 인증서의 SHA-1 해시 문자열을 표시합니다.
2. 액세스 지점 또는 컨트롤러 또는 AAA 서버 로그의 네트워크 패킷 캡처를 수집하여 EAP 세션이 실패하는 위치를 확인합니다.
    - HoloLens에서 제공하는 EAP ID가 필요하지 않은 경우 ID가 Wi-Fi 프로필 또는 클라이언트 인증서를 통해 올바르게 프로비전되었는지 확인합니다.
    - 서버에서 HoloLens 클라이언트 인증서를 거부하는 경우 필요한 클라이언트 인증서가 디바이스에 프로비전되었는지 확인합니다.
    - HoloLens에서 서버 인증서를 거부하는 경우 서버 루트 CA 인증서가 HoloLens에 프로비전되었는지 확인합니다.
...
1. Wi-Fi가 켜져 있는지 확인합니다. 시작 제스처를 사용하여 확인한 다음, 설정 > 네트워크 및 인터넷 > Wi-Fi를 차례로 선택합니다. Wi-Fi가 켜져 있으면 껐다가 다시 켜봅니다.
2. 라우터 또는 액세스 지점에 좀 더 가깝게 이동하세요.
3. Wi-Fi 라우터를 다시 시작한 다음, HoloLens를 다시 시작합니다. 다시 연결해 보세요.
4. 이러한 작업 중 아무것도 작동하지 않는 경우 라우터에서 최신 펌웨어를 사용하고 있는지 확인합니다. 이 정보는 제조업체 웹 사이트에서 확인할 수 있습니다.

디바이스에서 엔터프라이즈 또는 조직 계정에 로그인할 때 IT 관리자가 정책을 구성한 경우 MDM(모바일 장치 관리) 정책을 적용할 수도 있습니다.
...
1. PC에서 - 원격 컴퓨터가 연결할 수 있도록 Fiddler를 구성합니다.
    1. [Fiddler 설정 -> 연결]로 차례로 이동합니다.
    1. Fiddler에 대한 수신 포트를 적어 둡니다(기본값은 8866).
    1. [원격 컴퓨터 연결 허용]을 선택합니다.
    1. 저장을 클릭합니다.
3. HoloLens 2에서 - Fiddler를 프록시 서버<sup>1</sup>로 구성합니다.
    ...
    1. 위에서 적어 둔 포트 번호를 입력합니다(기본값은 8866).
    1. 저장을 클릭합니다.
...
1. PC에서 - Fiddler 인증서를 내보냅니다.
    1. [Fiddler 설정 -> HTTPS]로 차례로 이동하여 [고급 설정]을 펼칩니다.
    2. Fiddler 인증서 내보내기를 클릭합니다. 그러면 데스크톱에 저장됩니다.
    3. 인증서를 HoloLens 2의 [다운로드] 폴더로 이동합니다.

2.  HoloLens 2에서 - Fiddler 인증서를 가져옵니다.
    1. [설정 -> 업데이트 및 보안 -> 인증서]로 차례로 이동합니다.
    2. [인증서 설치]를 클릭하고, [다운로드] 폴더로 이동한 다음, Fiddler 인증서를 선택합니다.
    3. [저장소 위치]를 [로컬 컴퓨터]로 변경합니다.
    4. [인증서 저장소]를 루트로 변경합니다.
    5. 설치를 선택합니다.
    6. 인증서가 인증서 목록에 표시되는지 확인합니다. 그렇지 않으면 이러한 단계를 반복합니다.

#### <a name="inspect-https-sessions"></a>HTTP(S) 세션 검사

PC에서 Fiddler는 HoloLens 2 라이브 HTTP(S) 세션을 표시합니다. Fiddler의 [검사기] 패널은 HTTP(S) 요청/응답을 별도의 보기에 표시할 수 있습니다. 예를 들어, "원시" 보기는 원시 요청 또는 응답을 일반 텍스트로 표시합니다. 

### <a name="configure-wireshark-to-capture-network-traffic"></a>네트워크 트래픽을 캡처하도록 Wireshark 구성
Wireshark는 HoloLens 2 디바이스에서 들어오고 나가는 TCP/UDP 트래픽을 검사하는 데 사용되는 네트워크 프로토콜 분석기입니다. 이렇게 하면 네트워크를 통해 HoloLens 2로 이동하는 트래픽, 즉 트래픽의 양, 빈도, 특정 홉 사이의 대기 시간 등을 쉽게 식별할 수 있습니다.

#### <a name="prerequisites"></a>필수 조건:
- PC는 Wi-Fi를 통해 인터넷에 액세스하고 인터넷 공유를 지원해야 합니다.
...
영향을 받는 디바이스가 삭제된 후에 Azure AD 테넌트에 다시 연결하려고 하면 Azure AD에서 인증되지 않습니다. PIN을 통한 캐시된 로그온으로 사용자가 계속 로그온할 수 있으므로 이 효과가 장치 사용자에게 표시되지 않는 경우가 많습니다.
...
다음 문서는 자세한 내용을 알아보고 Autopilot 문제를 해결하는 데 유용한 리소스일 수 있습니다. 그러나 이러한 문서는 Windows 10 Desktop을 기반으로 하며, 모든 정보가 HoloLens에 적용될 수 있는 것은 아닙니다.
