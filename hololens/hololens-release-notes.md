...
> [!NOTE]
> 최근 Windows 11 알림은 PC 버전의 Windows에 초점을 두었습니다. 최근 2021 년 5 월 HoloLens 2에 대 한 [주요 OS 업데이트](https://techcommunity.microsoft.com/t5/mixed-reality-blog/what-s-new-in-windows-holographic-version-21h1/ba-p/2337067) 를 시작 했으며,이에 대 한 고객 의견을 바탕으로 예정 된 릴리스에 대해 작업 하 고 있습니다.
...
[디스플레이 색 보정](#display-color-calibration) | HoloLens 2 표시에 대 한 대체 색 프로필을 선택 합니다. | 최종 사용자 |
...
- [Microsoft Intune를 사용 하 여 정책 설정 Microsoft Edge 구성](/deployedge/configure-edge-with-intune)
- [Microsoft Edge 레거시 Microsoft Edge 정책 매핑](/deployedge/microsoft-edge-policy-map-legacy-to-newedge)
- [Google Chrome에서 정책 매핑 Microsoft Edge](/deployedge/microsoft-edge-policy-map-chrome-to-newedge)
- 전체 [Microsoft Edge Enterprise 설명서](/deployedge/)

> [!IMPORTANT]
> 새 Microsoft Edge에서 지원 되는 브라우저 정책의 볼륨 때문에 팀은 각 새 정책이 HoloLens 2에서 작동 하도록 보장할 수 없습니다. 그러나 이전에 HoloLens 2에서 지원 되는 각 레거시 Microsoft Edge 정책에 해당 하는 것과 동일한 새로운 Microsoft Edge를 테스트 하 고 확인 했습니다. HoloLens 2에서 사용 하는 각 레거시 Microsoft Edge 브라우저 정책에 해당 하는 새 Microsoft Edge를 찾으려면 [정책 매핑을 Microsoft Edge Microsoft Edge 레거시을](/deployedge/microsoft-edge-policy-map-legacy-to-newedge) 참조 하세요.
>
...
**장치에 직접 설치 (현재는 관리 되지 않는 장치 에서만 사용 가능)**
  1. HoloLens 2에서 [Edge 참가자 다운로드 페이지](https://www.microsoftedgeinsider.com/download)를 방문 하세요.
  1. 설치할 Edge Insider channel에 대해 **HoloLens 2 다운로드** 단추를 선택 합니다.
  1. Edge 다운로드 큐 또는 장치의 "다운로드" 폴더 (파일 탐색기 사용)에서 다운로드 한 msix 파일을 시작 합니다.
  1. [앱 설치 관리자](app-deploy-app-installer.md) 가 시작 됩니다.
  1. **설치** 단추를 선택합니다.
  1. 설치가 완료 되 면 시작 메뉴의 **모든 앱** 목록에서 별도의 항목으로 Microsoft Edge Beta, Dev 또는 카나리아를 찾을 수 있습니다.

**Windows 장치 포털을 사용 하 여 PC를 통해 설치 (HoloLens 2에서 [개발자 모드](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-windows-device-portal#setting-up-hololens-to-use-windows-device-portal) 를 사용 하도록 설정 해야 함)**
  1. PC에서 [Edge 참가자 다운로드 페이지](https://www.microsoftedgeinsider.com/download)를 방문 합니다.
  1. 설치할 Edge Insider channel의 "Windows 10 다운로드" 단추 옆에 있는 **드롭다운 화살표 단추** 를 선택 합니다.
  1. 드롭다운 메뉴에서 **HoloLens 2** 를 선택 합니다.
  1. PC의 "다운로드" 폴더 (또는 쉽게 찾을 수 있는 다른 폴더)에. msix 파일을 저장 합니다.
  1. PC에서 [Windows 장치 포털](/windows/mixed-reality/develop/platform-capabilities-and-apis/using-the-windows-device-portal#installing-an-app) 을 사용 하 여 HoloLens 2에서 다운로드 한 msix 파일을 설치 합니다.
  1. 설치가 완료 되 면 시작 메뉴의 **모든 앱** 목록에서 별도의 항목으로 Microsoft Edge Beta, Dev 또는 카나리아를 찾을 수 있습니다.

#### <a name="using-wdac-to-block-new-microsoft-edge"></a>WDAC를 사용 하 여 새 Microsoft Edge 차단

새 Microsoft Edge 앱을 차단 하도록 [WDAC 정책을](windows-defender-application-control-wdac.md) 업데이트 하려는 IT 관리자의 경우 정책에 다음을 추가 해야 합니다.

``` <Deny ID="ID_DENY_D_3_0" FriendlyName="C:\Data\Programs FileRule" PackageVersion="65535.65535.65535.65535" FileName="msedge.exe" /> ```

#### <a name="managing-endpoints-for-the-new-microsoft-edge"></a>새 Microsoft Edge의 끝점 관리

일부 환경에는 고려 사항으로 고려할 네트워크 제한이 있을 수 있습니다. 새에 지에 부드러운 환경을 보장 하려면 [이러한 Microsoft 끝점을 사용 하도록 설정 하세요.](/deployedge/microsoft-edge-security-endpoints)

[HoloLens에 대해](hololens-offline.md)현재 사용할 수 있는 끝점에 대해 자세히 알아보세요.

### <a name="install-web-apps"></a>웹 앱 설치
 > [!Note]
>[Windows Holographic 버전 21h1](hololens-release-notes.md#windows-holographic-version-21h1)에서는 Office 웹 앱이 더 이상 미리 설치 되지 않습니다.

새 Edge를 사용 하 여 Microsoft Store 앱과 함께 웹 앱을 설치할 수 있습니다. 예를 들어 Microsoft Office 웹 앱을 설치 하 여 SharePoint 또는 OneDrive에서 호스트 되는 파일을 보고 편집할 수 있습니다. Office 웹 앱을 설치 하려면를 방문 하 여 https://www.office.com 주소 표시줄에서 **앱 사용 가능** 또는 **설치 Office** 단추를 선택 합니다. **설치** 를 선택 하 여 확인 합니다.
...
새 Microsoft Edge에는 몰입 형 웹 환경을 만들기 위한 새로운 표준 인 WebXR에 대 한 지원이 포함 되어 있습니다 (WebVR 대체). 대부분의 몰입 형 웹 환경은 VR를 염두에 두어야 설계 되었지만 (이는 보기의 필드를 가상 환경으로 대체), 이러한 환경은 HoloLens 2 에서도 지원 됩니다. WebXR standard를 사용 하면 물리적 환경을 사용 하는 확장 및 혼합 현실 몰입 형 웹 환경을 사용할 수도 있습니다. 개발자가 WebXR를 사용 하 여 더 많은 시간을 소비 하는 경우, 고객이 시험해 볼 수 있는 새로운 확장 및 혼합 현실 환경이 HoloLens 2 도착할 것으로 예상 됩니다!

360 뷰어 확장은 WebXR를 기반으로 하며 HoloLens 2에 새 Microsoft Edge 함께 자동으로 설치 됩니다. 이 웹 확장은 360 수준 비디오에서 직접 컨퍼런스 수 있는 기능을 제공 합니다. YouTube는 가장 광범위 한 360 비디오를 제공 하므로 시작 하는 것이 좋습니다.
...
새 Microsoft Edge에서 **사용자 의견 보내기** 기능을 통해 팀과 의견 및 버그를 공유 하세요.

### <a name="new-settings-app"></a>새 설정 앱

이 릴리스에서는 설정 앱의 새 버전을 소개 하 고 있습니다. 새 설정 앱에는 소리, 전원 & 절전, 네트워크 & 인터넷, 앱, 계정, 액세스 편의성 등의 영역에서 HoloLens 2에 대 한 새로운 기능 및 확장 된 설정이 포함 되어 있습니다.

> [!NOTE]
> 새 설정 앱은 레거시 설정 앱과 다르기 때문에 이전에 환경 주위에 배치한 모든 설정 windows는 업데이트 시 제거 됩니다.

![새 설정 app 홈페이지](images/new-settings-app.png)

**새 기능 및 설정**
- 설정 검색: 키워드 또는 설정 이름을 사용 하 여 설정 홈 페이지에서 설정을 검색 합니다.
- 시스템 > 소리:
  - 입력 및 출력 오디오 장치: 독립적으로 입력 및 출력 오디오 장치를 선택 합니다. 예를 들어 Bluetooth 헤드폰을 통해 오디오를 수신 하거나 오디오 입력에 USB-C 마이크를 사용 합니다.
    > [!NOTE]
...
**알려진 문제**
- 이전에 배치한 설정 창이 제거 됩니다 (위의 참고 참조).
- 더 이상 설정 앱으로 장치 이름을 바꿀 수 없습니다. IT 관리자는 HoloLens 2 장치 이름 템플릿 또는 MDM [devdetail CSP](/windows/client-management/mdm/devdetail-csp) Ext/Microsoft/dnscomputername 노드의 [Windows Autopilot](hololens2-autopilot.md) 를 사용 하 여 장치 이름을 바꿀 수 있습니다.
- 이더넷 페이지에는 항상 가상 이더넷 장치 ("UsbNcm")가 표시 됩니다.
- 새 Microsoft Edge에 대 한 배터리 사용량은 UWP 어댑터 계층에서 지원 되는 Win32 데스크톱 응용 프로그램의 특성으로 인해 정확 하지 않을 수 있습니다 (곧 수정 되지 않음).

#### <a name="display-color-calibration"></a>디스플레이 색 보정



이 새로운 설정을 사용 하 여 HoloLens 2 표시에 대 한 대체 색 프로필을 선택할 수 있습니다. 이렇게 하면 색이 더 정확 하 게 표시 될 수 있습니다. 특히 디스플레이 밝기 수준이 낮습니다. 디스플레이 색 보정은 시스템 > 보정 페이지의 설정 앱에서 찾을 수 있습니다.
...
1. 명령 대화 상자를 진행 한 후에는 디스플레이가 자동으로 30% 밝기까지 흐리게 표시 됩니다.
    > [!TIP]
    > 사용자 환경에서 흐린 장면을 표시 하는 데 문제가 있는 경우 장치의 왼쪽에 있는 밝기 단추를 사용 하 여 HoloLens 2의 밝기 수준을 수동으로 조정할 수 있습니다.
...
> - 장치의 모든 사용자가 이전에이 설정을 사용 하 여 색 프로필을 변경한 경우 최신 변경의 날짜/시간이 설정 페이지에 반영 됩니다.
> - 표시 색 보정을 다시 실행 하면 이전에 저장 된 색 프로필이 강조 표시 되 고 프로필 0은 표시 되지 않습니다 (프로필 0은 표시의 원래 색 프로필을 나타냄).
> - 디스플레이의 원래 색 프로필로 되돌리려면 설정 페이지에서 [색 프로필을 다시 설정 하는 방법](#how-to-reset-color-profile)을 참조 하세요.)
...
"Always"를 선택 했지만 나중에 특정 파일 또는 링크 형식을 처리 하는 앱을 변경 하려는 경우 **설정 > 앱** 에서 저장 된 기본값을 다시 설정할 수 있습니다. 페이지 아래쪽으로 스크롤하여 "파일 형식에 대 한 기본 앱" 및/또는 "링크 형식에 대 한 기본 앱" 아래에서 **지우기** 단추를 선택 합니다. 데스크톱 Pc의 비슷한 설정과 달리 개별 파일 유형 기본값은 다시 설정할 수 없습니다.

#### <a name="per-app-volume-control"></a>앱 당 볼륨 컨트롤

이제이 Windows 빌드에서 사용자는 각 앱의 볼륨 수준을 수동으로 조정할 수 있습니다. 이렇게 하면 사용자가 필요한 앱에 더 잘 집중 하거나 여러 앱을 사용 하는 경우 더 잘 듣지 못합니다. 다른 사용자가 다른 사용자의 원격 지원을 위해 다른 사용자를 호출 하는 동안 한 앱의 볼륨을 해제 해야 합니다.

개별 앱의 볼륨을 설정 하려면 **설정**  ->  **시스템**  ->  **소리로** 이동 하 고 고급 소리 옵션에서 **앱 볼륨 및 장치 기본 설정** 을 선택 합니다.<br/><br/>
...
2. 사용자 프로필 그림 옆에 있는 줄임표 아이콘 (...)을 확인 합니다.<br/><br/>

   <img alt="ser context dots, ..." src="./images/powertransition_icon_default_cropped.png" width="586" height="330" />

3. 손으로 나 음성 명령 "Power"를 사용 하 여 사용자 프로필 그림을 선택 합니다.

4. 로그 아웃 하 고 장치를 다시 시작 하거나 종료 하는 옵션이 포함 된 메뉴가 나타납니다.<br/><br/>
...
6. 다른 위치에 접촉 하거나 시작 제스처를 사용 하 여 시작 메뉴 닫는 방법으로 메뉴를 해제 합니다.
...
> [!IMPORTANT]
> **USB mic를 꽂으면 입력 장치로 자동 설정 되지** 않습니다. USB-C 헤드폰 사용자를 연결 하는 경우 헤드폰의 오디오가 자동으로 헤드폰으로 리디렉션되고 HoloLens OS는 다른 입력 장치 위에 있는 내부 마이크 배열의 우선 순위를 설정 합니다. **USB-C 마이크를 사용 하려면 다음 단계를 수행 합니다.**
...
>
> **통신 기본값** 을 선택 하면 외부 마이크가 원격 지원 및 기타 통신 앱에서 사용 되지만 다른 작업에는 HoloLens mic 배열을 사용할 수 있습니다.
...
[Windows Holographic 버전 20H2에서는](hololens-release-notes.md#windows-holographic-version-20h2) 설정 앱 내에서 볼 수 있는 페이지를 제한하는 [설정/PageVisibilityList 정책을](/windows/client-management/mdm/policy-csp-settings#settings-pagevisibilitylist) 추가했습니다. PageVisibilityList는 IT 관리자가 System 설정 앱의 특정 페이지를 표시하거나 액세스할 수 없도록 하거나 지정된 페이지를 제외한 모든 페이지에 대해 이를 수행할 수 있도록 하는 정책입니다.
...
PC 및 기타 HoloLens 2 디바이스를 포함하여 Windows 10 디바이스에서 가까운 디바이스와 사물을 공유합니다. 설정 시스템 공유   ->    ->  **경험에서** 시도하여 HoloLens PC로 파일 또는 URL을 공유할 수 있습니다. 자세한 내용은 Windows 10 에서 [주변 디바이스와 사물을 공유하는](https://support.microsoft.com/windows/share-things-with-nearby-devices-in-windows-10-0efbfe40-e3e2-581b-13f4-1a0e9936c2d9)방법에 대해 자세히 읽어 읽습니다.
...
이 HoloLens 업데이트를 사용하면 Windows Holographic for Business 전송 최적화 설정을 통해 여러 HoloLens 디바이스에서 다운로드하는 데 대역폭 사용량을 줄일 수 있습니다. 권장되는 네트워크 구성과 함께 이 기능에 대한 자세한 설명은 [Windows 10 업데이트에 대한 배달 최적화에서](/windows/deployment/update/waas-delivery-optimization)확인할 수 있습니다.

다음 설정은 관리 표면의 일부로 사용하도록 설정되며 [Intune 에서 구성할 수 있습니다.](/mem/intune/configuration/delivery-optimization-settings)
...
- Windows Holographic for Business [Microsoft 연결된 캐시 엔드포인트에서](/mem/configmgr/core/plan-design/hierarchy/microsoft-connected-cache)HTTP 다운로드 모드 및 다운로드만 지원합니다. 피어 투 피어 다운로드 모드 및 그룹 할당은 현재 HoloLens 디바이스에서 지원되지 않습니다.
- HoloLens Windows Server Update Services 엔드포인트에 대한 배포 또는 배달 최적화를 지원하지 않습니다.
- 문제를 해결하려면 연결된 캐시 서버에서 진단하거나 설정 업데이트 & 보안 문제 해결 Windows 업데이트를 통해 **HoloLens** HoloLens 대한  >    >   **추적을** 수집해야  >   합니다.
...
- 연구 모드 에 대한 arm32 지원이 [추가되었습니다.](/windows/mixed-reality/develop/platform-capabilities-and-apis/research-mode)
...
HoloLens 2에서 눈 위치는 홀로그램 위치 지정, 편안 하 게 보기 경험 및 향상 된 디스플레이 품질을 제공 합니다. 눈 위치는 내부적으로 눈 추적 계산의 일부로 계산 됩니다. 그러나이를 위해서는 각 사용자가 눈에 눈길을 요하는 입력이 필요 하지 않은 경우에도 눈 추적 보정을 거쳐야 합니다.
...
Windows Holographic 버전 20h2에서 HoloLens 2 설정 앱에 인증서 관리자를 추가 합니다. 설정 > **업데이트 & 보안 > 인증서로 이동합니다.** 이 기능은 디바이스에서 인증서를 보고, 설치하고, 제거하는 간단하고 사용자에게 친숙한 방법을 제공합니다. 이제 새 인증서 관리자를 통해 관리자와 사용자가 감사, 진단 및 유효성 검사 도구를 개선하여 디바이스가 안전하고 규정을 준수하는지 확인합니다. 
...
[TenantLockdown](/windows/client-management/mdm/tenantlockdown-csp) CSP를 사용하면 HoloLens 2 Autopilot만 사용하여 MDM 등록에 연결될 수 있습니다. TenantLockdown CSP의 RequireNetworkInOOBE 노드가 HoloLens 2 true 또는 false(처음에 설정) 값으로 설정되면 해당 값은 다시 플래시, OS 업데이트 등에도 불구하고 디바이스에 유지됩니다. 
...
1. 이전 단계에서 만든 그룹의 HoloLens 2 디바이스 멤버를 만들고 동기화를 트리거합니다.  
...
#### <a name="what-would-happen-during-oobe-if-autopilot-profile-is-unassigned-on-a-hololens-after-tenantlockdown-was-set-to-true"></a>TenantLockdown이 true로 설정된 후 HoloLens Autopilot 프로필이 할당되지 않은 경우 OOBE 중에 어떻게 됩니까? 
...
- Unity 애플리케이션용 HolographicSpace.UserPresence API를 일시적으로 사용하지 않도록 설정했습니다. 이 변경으로 인해 "백그라운드에서 실행" 설정이 설정된 경우에도 바이저가 대칭 이동되었을 때 일부 앱이 일시 중지되는 문제를 방지할 수 있습니다.
- 손 추적으로 인한 임의 HUP 크래시를 수정했습니다. 이 경우 사용자는 몇 초 후에 UI가 고정된 후 다시 셸로 이동했습니다.
...
- LSR(마지막 단계 재현) 수정이 도입되었습니다. 깊이를 보다 정확하게 고려하여 홀로그램의 시각적 렌더링이 더 안정적이고 선명하게 표시되었습니다. 앱이 홀로그램의 깊이를 올바르게 설정하지 않으면 이 업데이트 후에 이 증상이 더 두드러질 것입니다.
- 배타적 앱의 안정성과 배타적 앱 간의 탐색이 수정되었습니다.
- 디바이스가 며칠 동안 대기 상태이면 혼합 현실 캡처가 비디오를 녹화할 수 없는 문제가 해결되었습니다.
- 홀로그램 안정성이 향상되었습니다.
