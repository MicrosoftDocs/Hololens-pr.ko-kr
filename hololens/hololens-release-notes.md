...
## <a name="windows-holographic-version-21h1---september-2021-update"></a>Windows 홀로그램 버전 21H1 - 2021년 9월 업데이트

- 빌드 20348.1018

업데이트의 개선 사항 및 수정 사항:

- 시스템 시간이 예기치 않게 이동할 수 있는 문제를 해결하기 위한 수정

## <a name="windows-holographic-version-20h2---september-2021-update"></a>Windows 홀로그램 버전 20H2 - 2021년 9월 업데이트
...
## <a name="windows-holographic-version-21h1---august-2021-update"></a>Windows 홀로그램 버전 21H1 - 2021년 8월 업데이트

- 빌드 20348.1014

업데이트의 개선 사항 및 수정 사항:

- Xbox 컨트롤러가 컨트롤러 지원을 통해 몰입형 애플리케이션에서 작동하지 않는 문제를 해결했습니다.
- 디바이스 업데이트 실패에 대한 진단이 향상되었습니다.

## <a name="windows-holographic-version-20h2---august-2021-update"></a>Windows 홀로그램 버전 20H2 - 2021년 8월 업데이트
...
- Wi-Fi 속성 UI가 설정 > Network & Internet > 상태 > 속성에서 시작될 때 Wi-Fi 프록시를 저장할 수 없는 문제가 **해결되었습니다.**
...
- CPU 로드가 증가하면서 충전 시 런타임을 늘리도록 배터리 충전 성능이 조정되었습니다. HoloLens 2 디바이스를 충전하는 동안 디바이스가 핫으로 실행되는 것으로 감지되면 내부 배터리가 더 느리게 충전되어 열을 줄입니다. 긍정적인 단점은 열 문제로 인해 디바이스를 종료할 가능성이 적다는 것입니다. 그 영향은 디바이스가 더 오래 실행된다는 것입니다. 디바이스가 쿨로 실행되는 경우 요금에는 영향을 받지 않습니다.
...
### <a name="onedrive-for-work-or-school-camera-roll-upload"></a>직장 또는 학교 카메라 롤 업로드에 대한 OneDrive

HoloLens 2 설정 앱에 새로운 기능이 추가되었습니다. 이 기능을 통해 고객은 디바이스의 사진 > Camera Roll 폴더에서 혼합 현실 사진 및 비디오를 해당 OneDrive 또는 학교 폴더에 자동으로 업로드할 수 있습니다. 이 기능은 [HoloLens 2 OneDrive 앱 내에서 기능 차이를](holographic-photos-and-videos.md#share-your-mixed-reality-photos-and-videos) 해결합니다. 이 앱은 고객의 개인 Microsoft 계정 자동 카메라 롤 업로드만 지원합니다(직장 또는 학교 계정이 아닌).
...
- 설정 페이지의 업로드 횟수를 실시간으로 보는 동안 사진이나 비디오를 찍는 경우 현재 파일 업로드가 완료될 때까지 보류 중인 파일 수가 변경되지 않을 수 있습니다.
- 디바이스가 절전 상태이거나 전원이 꺼지면 업로드 일시 중지됩니다. 보류 중인 업로드가 완료되도록 하려면 설정 페이지에 "OneDrive 최신 상태"가 표시될 때까지 디바이스를 적극적으로 사용하거나 **Power & 절전 모드** 설정을 조정합니다.
...
이전 빌드에서 장치에 할당 된 전역 액세스 권한 및 AAD 그룹 구성원 모두의 조합 인 키오스크 구성이 있는 경우 AAD 그룹 멤버 자격을 확인 하지 못한 경우 사용자에 게 "[시작에 표시 되지 않음](hololens-kiosk.md#issue---no-apps-are-shown-in-start-menu-in-kiosk-mode)" 메뉴가 표시 됩니다.
...
✔️ [대체 진단 구성:](#configuring-fallback-diagnostics-via-settings-app)대체 진단을 수집할 수 있는지와 누가 수집할 수 있는지 구성할 수 있습니다.
...
- 장치 포털 Service에서 메모리 누수 문제가 해결되면서 다른 애플리케이션이 메모리를 할당하지 못하는 서비스로 인해 메모리 사용량이 증가했습니다.
...
Windows 유니버설 샘플 [GitHub 샘플](https://github.com/microsoft/Windows-universal-samples/tree/master/Samples) 앱을 찾아 이 흐름을 테스트할 수 있습니다.
...
- 이제 손 추적은 이전에 손을 잃을 수 있었던 많은 새로운 경우에 추적을 유지 관리합니다.  이러한 새로운 경우 중 일부에서는 손만 위치만 사용자의 실제 손을 기반으로 계속 업데이트되고 다른 공동은 이전 자세를 기반으로 유추됩니다.  이 변경은 이동에서 추적 일관성을 개선하는 데 도움이 됩니다(예: 트래핑, throw, 힌팅 및 힌팅).  또한 손은 표면에 가깝거나 개체를 보유하는 경우에도 도움이 됩니다.  손 조인을 유추하는 경우 [공동 정확도당](/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) 값은 "높음" 대신 "근사값"으로 설정됩니다.
...
| [USB에서 자동 시작 프로비저닝](hololens-release-notes.md#auto-launch-provisioning-from-usb)                    | USB 드라이브에서 패키지를 프로비전하는 경우 OOBE의 프로비저닝 페이지가 자동으로 표시됩니다.                                                         |
...
#### <a name="steps-to-install-a-certificate"></a>인증서를 설치 하는 단계
...
#### <a name="steps-to-remove-a-certificate"></a>인증서를 제거 하는 단계
...
![인증서 UI를 사용 하 여 인증서를 설치 하는 방법을 보여 주는 그림입니다.](images/certificate-device-install.jpg)
...
이 릴리스 이전에는 사용자가 OOBE에서 단추 조합을 사용 하 여 프로 비전 하는 수동으로 프로 비전 화면을 시작 해야 했습니다. 이제 사용자가 USB 저장소 드라이브에서 프로 비전 패키지를 사용 하 여 단추 조합을 건너뛸 수 있습니다.
...
이 정보는 이제 [Tenantlockdown CSP 및 Autopilot](hololens2-autopilot.md#tenant-lockdown-csp-and-autopilot)아래 Autopilot의 나머지 부분과 함께 찾을 수 있습니다.
...
이 새로운 기능을 사용 하면 IT 관리자가 시스템 수준에서 적용할 수 있는 여러 앱 키오스크 모드에 대 한 HoloLens 2 장치를 구성 하 고, 시스템의 id로 선호도를 적용 하지 않으며, 장치에 로그인 하는 모든 사용자에 게 적용할 수 있습니다. 이 새로운 기능에 대 한 자세한 내용은 [HoloLens 키오스크 모드](hololens-kiosk.md)를 참조 하세요.
...
- Wi-Fi 연결을 통해 장치 포털 액세스하는 경우 웹 브라우저에서 잘못된 인증서로 인해 에 액세스하지 못할 수 있습니다. 이전에 디바이스 인증서를 신뢰한 경우에도 브라우저에서 "ERR_SSL_PROTOCOL_ERROR"과 같은 오류를 보고할 수 있습니다. 이 경우 보안 경고를 무시하는 옵션이 없기 때문에 장치 포털 진행할 수 없습니다. 이 업데이트로 문제가 해결되었습니다. 브라우저 보안 경고를 제거하기 위해 이전에 PC에서 디바이스 인증서를 다운로드하고 신뢰했으며 SSL 오류가 발생하는 경우 브라우저 보안 경고를 해결하려면 새 인증서를 다운로드하고 신뢰해야 합니다.
...
- 아이리스 로그인 중 충돌을 일으키는 버그가 수정되었습니다.
...
- 1903 릴리스에서 업데이트한 후 초기 부팅에서 사진 앱의 시작에 대한 문제가 해결되었습니다.

## <a name="windows-holographic-version-2004"></a>Windows 홀로그램 버전 2004

- 빌드 - 19041.1103

HoloLens 2, *Windows Holographic 버전 2004에 대한 2020년 5월* 주요 소프트웨어 업데이트에는 Windows Autopilot 지원, 앱 어둡게 모드, 5G/LTE 핫스팟에 대한 USB 이더넷 지원 등과 같은 흥미로운 새로운 기능이 포함되어 있습니다. 최신 릴리스로 업데이트하려면 **설정** 앱을 열고   업데이트  **& 보안으로** 이동하여 업데이트  **확인**   단추를 선택합니다. 

|             기능                              |          설명                                                                                              |
|--------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
|       Windows Autopilot                          |          Windows AutoPilot을 사용하여 프로덕션을 위한 새 디바이스를 미리 구성하고 원활하게 설정합니다.                 |
...
Windows HoloLens 2 Autopilot을 사용하면 디바이스 판매 채널이 intune 테넌트에서 HoloLens 미리 등록할 수 있습니다. 디바이스가 도착하면 테넌트에서 공유 디바이스로 자체 배포할 준비가 된 것입니다. 자체 배포를 활용하려면 디바이스가 USB-C-이더넷을 사용하여 설치의 첫 번째 화면 중에 네트워크에 연결해야 합니다.
...
USB를 통해 HoloLens 2 테더링될 때 5G/LTE 휴대폰 및 Wi-Fi 핫스팟과 같은 특정 모바일 광대역 디바이스를 사용할 수 있도록 지원이 추가되었습니다. 이러한 디바이스는 이제 **네트워크 설정에** 다른 이더넷 연결로 표시됩니다. (외부 드라이버가 필요한 모바일 광대역 디바이스는 지원되지 않습니다.) 이 기능은 Wi-Fi 사용할 수 없고 Wi-Fi 테더링 성능이 충분하지 않을 때 고대역폭 연결을 사용하도록 설정합니다. 지원되는 USB 디바이스에 대한 자세한 내용은 [Bluetooth 및 USB-C 디바이스에 대한 커넥트 참조하세요.](hololens-connect-devices.md)  
...
이제 많은 Windows 앱에서 어둡게 모드와 밝은 모드를 모두 지원합니다. HoloLens 2 사용자는 둘 다 지원하는 앱의 기본 모드를 선택할 수 있습니다. 업데이트 후 기본 앱 모드는 "어둡게"이지만 이 설정을 쉽게 변경할 수 있습니다. **설정**  >  **시스템**  >  **색으로**  >  **이동합니다. 기본 앱 모드를 선택합니다.**
...
- HoloLens Wi-Fi 스트리밍이 주기적으로 중단되는 버그가 수정되었습니다. 애플리케이션에서 짧은 대기 시간 스트리밍이 필요하다고 표시되면 [SetSocketMediaStreamingMode 함수](/windows/win32/api/socketapi/nf-socketapi-setsocketmediastreamingmode)를 호출하여 수정 사항을 구현합니다.
...
많은 Windows 앱은 어둡게 및 밝은 모드를 모두 지원합니다. HoloLens 2 고객은 이제 두 색 구성표를 모두 지원하는 앱의 기본 모드를 선택할 수 있습니다. 고객 피드백에 따라 기본 앱 모드를 "어둡게"로 설정하지만 언제든지 이 설정을 쉽게 변경할 수 있습니다. **설정 > 시스템 > 색으로** 이동하여 **"기본 앱 모드 선택"을** 찾습니다.
...
- 디바이스가 며칠 동안 대기 상태이면 혼합 현실 캡처가 비디오를 녹화할 수 없는 문제를 해결했습니다.
- 홀로그램 안정성이 향상되었습니다.
