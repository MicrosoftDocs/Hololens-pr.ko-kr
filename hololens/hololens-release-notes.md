...
USB 디바이스에서 프로비전의 자동 시작과 패키지 프로비전 자동 확인을 결합하여 사용자는 디바이스의 UI를 사용하거나 디바이스를 사용하지 않고도 HoloLens 2 디바이스를 자동으로 프로비전할 수 있습니다. 여러 디바이스에 대해 동일한 USB 드라이브 및 프로비전 패키지를 계속 사용할 수 있습니다. 이 방법은 동일한 영역에서 여러 디바이스를 한 번에 배포하는 데 유용합니다. 
...
- Autopilot이 Wi-Fi 연결된 디바이스에서 작동할 수 있도록 하여 USB-C 어댑터가 이더넷에 필요를 제거하여 하드웨어 요구 사항을 줄입니다.
...
이 새로운 기능을 사용하면 IT 관리자가 여러 앱 키오스크 모드에 대해 HoloLens 2 디바이스를 구성할 수 있습니다. 이 모드는 시스템 수준에서 적용할 수 있으며 시스템의 ID와 선호도가 없으며 디바이스에 서명하는 모든 사람에게 적용됩니다. [HoloLens 키오스크 모드에서](hololens-kiosk.md)이 새로운 기능에 대해 자세히 읽어보십시오.
...
이전에 키오스크 모드를 적용할 때 오류가 발생한 HoloLens 시작 메뉴에 모든 애플리케이션을 표시하는 데 사용되었습니다. 이제 Windows Holographic 버전 20H2에서 오류가 발생한 경우 아래와 같이 시작 메뉴에 앱이 표시되지 않습니다.
...
- Wi-Fi 연결을 통해 장치 포털 액세스하는 경우 웹 브라우저에서 잘못된 인증서로 인해 에 액세스하지 못할 수 있습니다. 이전에 디바이스 인증서를 신뢰한 경우에도 브라우저에서 "ERR_SSL_PROTOCOL_ERROR"와 같은 오류를 보고할 수 있습니다. 이 경우 보안 경고를 무시하는 옵션이 없기 때문에 장치 포털 진행할 수 없습니다. 이 업데이트로 문제가 해결되었습니다. 브라우저 보안 경고를 제거하기 위해 이전에 PC에서 디바이스 인증서를 다운로드하고 신뢰했으며 SSL 오류가 발생하는 경우 브라우저 보안 경고를 해결하려면 새 인증서를 다운로드하고 신뢰해야 합니다.
...
|       향상된 프로비전                      |          USB 드라이브에서 HoloLens 프로비전 패키지를 원활하게 적용                              |
...
Windows HoloLens 2 Autopilot을 사용하면 디바이스 판매 채널이 intune 테넌트에서 HoloLens 미리 등록할 수 있습니다. 디바이스가 도착하면 테넌트에서 공유 디바이스로 자체 배포할 준비가 된 것입니다. 자체 배포를 활용하려면 USB-C-이더넷을 사용하여 설치의 첫 번째 화면 중에 디바이스가 네트워크에 연결해야 합니다.
...
일부 사용자는 HoloLens 디바이스를 직장 또는 학교 환경의 다른 사용자와 공유합니다. 따라서 사용자가 긴 사용자 이름과 암호를 입력하지 않고도 쉽게 할 수 있는 것이 중요합니다. FIDO(Fast Identity Online)를 사용하면 조직(Azure AD 테넌트)의 모든 사용자가 사용자 이름이나 암호를 입력하지 않고도 HoloLens 원활하게 로그인할 수 있습니다.
...
패키지를 프로비전하면 HoloLens 바로 사용할 수 있는 환경이 아닌 구성 파일을 통해 HoloLens 구성을 설정할 수 있습니다. 이전에는 프로비전 패키지를 HoloLens 내부 메모리에 복사해야 했습니다. 이제 USB 드라이브에 있을 수 있으므로 여러 HoloLens 디바이스에서 더 쉽게 다시 사용할 수 있으며 동시에 디바이스를 프로비전할 수 있습니다. 프로 비전 패키지는 이제 장치 관리에 등록할 필드도 지원 하므로 프로 비전 후 수동 설정이 없습니다.
