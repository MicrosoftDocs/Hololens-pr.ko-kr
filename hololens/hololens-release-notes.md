...
HoloLens 2 디바이스의 더 많은 관리 옵션을 허용하는 새로 사용하도록 설정된 정책입니다. 
...
- 특정 디바이스에서 하드웨어 가속과 함께 HoloLens Emulator 사용하는 동안 배타적 HoloLens 앱이 셸에 충돌하는 문제를 해결했습니다.
...
- Wi-Fi 연결을 통해 장치 포털 액세스하는 경우 웹 브라우저에서 잘못된 인증서로 인해 에 액세스하지 못할 수 있습니다. 이전에 디바이스 인증서를 신뢰한 경우에도 브라우저에서 "ERR_SSL_PROTOCOL_ERROR"과 같은 오류를 보고할 수 있습니다. 이 경우 보안 경고를 무시하는 옵션이 없으면 장치 포털 진행할 수 없습니다. 이 업데이트로 문제가 해결되었습니다. 브라우저 보안 경고를 제거하기 위해 이전에 PC에서 디바이스 인증서를 다운로드하고 신뢰했으며 SSL 오류가 발생하는 경우 브라우저 보안 경고를 해결하려면 새 인증서를 다운로드하고 신뢰해야 합니다.
...
- HoloLens 앱에서 픽셀 형식을 변경하여 HoloLens Emulator 검은색으로 렌더링하는 문제를 해결했습니다.
...
FIDO2 보안 키는 폼 팩터에서 사용할 수 있는 "암호화할 수 없는" 표준 기반 암호 없는 인증 방법입니다. FIDO는 암호 없는 인증을 위한 개방형 표준입니다. 사용자와 조직이 사용자 이름 또는 암호 없이 해당 리소스에 로그인 할 수 있습니다. 대신 장치에 기본 제공 되는 외부 보안 키 또는 플랫폼 키를 사용 합니다.
...
- Unreal 개발자는 이제 장치 포털 3D 보기 페이지를 사용하여 애플리케이션을 테스트하고 디버그할 수 있습니다.
