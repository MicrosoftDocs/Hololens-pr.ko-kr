...
> [!NOTE]
> HoloLens 2에서 새 Microsoft Edge를 처음 시작하면 기존 Microsoft Edge에서 설정 및 데이터를 가져옵니다. 새 Microsoft Edge 시작한 후 레거시 Microsoft Edge 계속 사용하는 경우 해당 새 데이터가 레거시 Microsoft Edge 새 Microsoft Edge 동기화되지 않습니다.
...
- 이제 손 추적은 이전에 손을 잃을 수 있었던 많은 새로운 경우에 추적을 유지 관리합니다.  이러한 새로운 경우 중 일부에서는 손만 위치만 사용자의 실제 손을 기반으로 계속 업데이트되고 다른 공동은 이전 자세를 기반으로 유추됩니다.  이 변경은 이동에서 추적 일관성을 개선하는 데 도움이 됩니다(예: 싱, throw, 싱, 힌팅).  또한 손은 표면에 가깝거나 개체를 보유하는 경우에도 도움이 됩니다.  손 조인을 유추하는 경우 [공동 정확도당](/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) 값은 "높음" 대신 "근사값"으로 설정됩니다.
