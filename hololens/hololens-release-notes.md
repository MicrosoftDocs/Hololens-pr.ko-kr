...
- 손 추적은 이제 이전에 손을 잃은 많은 새로운 경우 추적을 유지 관리합니다.  이러한 새로운 경우 일부에서는 사용자의 실제 손바니에 따라 손바지 위치만 업데이트되는 반면 다른 조인트는 이전 포즈에 따라 유추됩니다.  이 변경은 슬래시, 던지기, 스푸핑 및 박수와 같은 동작의 추적 일관성을 향상시키는 데 도움이 됩니다.  손이 표면에 가깝거나 개체를 잡고 있는 경우도 도움이 됩니다.  손 조인트가 유추되는 경우 [per joint accuracy](https://docs.microsoft.com/uwp/api/windows.perception.people.jointposeaccuracy?view=winrt-19041&preserve-view=true) 조인트당 정확도 값은 "높음" 대신 "대략값"으로 설정됩니다.
...
| [오프라인 Kiosk에 대한 AAD 그룹 구성원 캐시](hololens-release-notes.md#cache-aad-group-membership-for-offline-kiosk)         | 새 정책을 사용하면 사용자가 그룹 구성원 자격 캐시를 사용하여 설정된 일 수 동안 키오스크 모드를 오프라인으로 사용할 수 있습니다.                                        |
...
이 새로운 기능을 통해 IT 관리자는 시스템 수준에서 적용할 수 있는 여러 앱 키오스크 모드에 대해 HoloLens 2 장치를 구성할 수 있으며, 시스템의 ID와 관련이 없습니다. 이 디바이스에 로그인하는 모든 사용자에 적용할 수 있습니다. 이 새로운 기능에 대한 자세한 내용은 여기를 [참조하세요.](hololens-global-assigned-access-kiosk.md)
...
| MixedReality\AADGroupMembershipCacheValidityInDays | AAD 그룹을 대상으로 하는 Kiosk에 AAD 그룹 구성원 자격 캐시가 사용되는 일 수를 제어합니다. | 아래를 참조하세요.                                                           |

### 오프라인 Kiosk에 대한 AAD 그룹 구성원 캐시
- 오프라인 키오스크를 최대 60일 동안 AAD 그룹과 함께 사용할 수 있습니다.

이 정책은 로그인한 사용자의 AAD 그룹을 대상으로 하는 할당된 액세스 구성에 AAD 그룹 구성원 캐시를 사용할 수 있는 일 수를 제어합니다. 이 정책 값을 0보다 큰 값으로 설정하면 캐시가 사용되지 않습니다.  
...
이 정책을 올바르게 사용하는 단계: 
1. AAD 그룹을 대상으로 하는 키오스크에 대한 장치 구성 프로필을 만들고 HoloLens 장치에 할당합니다. 
...
1. 인터넷을 사용할 수 있는 경우 AAD 사용자가 로그인하고 AAD 그룹 구성원이 확인되면 캐시가 만들어집니다. 
1. 이제 AAD 사용자 1은 HoloLens를 오프라인으로 전환하고 정책 값이 X일 수를 허용하는 한 키오스크 모드에 사용할 수 있습니다. 
1. 다른 AAD 사용자 N에 대해 4단계와 5단계를 반복할 수 있습니다. 여기서는 키오스크 구성의 대상이 될 AAD 그룹의 구성원인지 적어도 한 번은 AAD 사용자가 인터넷을 사용하여 장치에 로그인해야 합니다. 
 
> [!NOTE]
> AAD 사용자에 대해 4단계를 수행할 때까지 "연결이 끊어진" 환경에 언급된 오류 동작이 발생합니다. 
...
- AAD 그룹을 대상으로 하는 키오스크 모드와의 HTTP 캐싱으로 인한 문제가 해결되었습니다.
...
- 사용자가 디바이스가 **Settings**종료될 때 Mixed Reality 홈에서 모든 홀로그램을 자동으로 제거할 수 있도록 하는 설정 시스템 홀로그램에  >  **System**  >  **Holograms** 설정이 추가되었습니다.
...
HoloLens에는 업무용 앱에 대한 MDM 앱 배포 및 관리가 중요합니다. 관리자와 사용자는 감사 및 진단을 위해 앱 설치 상태를 볼 수 있습니다. 이 릴리스에서는 설정 계정 액세스 **Settings**직장 또는 학교에서 계정 정보 클릭에 더 많은 세부  >  **Accounts**  >  **Access work or school**  >  **정보를**  >  **추가했습니다.**
...
이제 많은 Windows 앱이 어둡게 모드와 밝은 모드를 모두 지원하고 있습니다. HoloLens 2 사용자는 둘 다 지원하는 앱의 기본 모드를 선택할 수 있습니다. 업데이트 후 기본 앱 모드는 "어둡게"이지만 설정 시스템 **Settings**색으로 이동하여 기본 앱 모드를 선택합니다.  >  **System**  >  **Colors**  >  **Choose your default app mode** 
...
- 마지막 단계 재생(LSR) 수정을 도입했습니다. 홀로그램의 시각적 렌더링이 향상되어 깊이를 보다 정확하게 고려하여 더욱 안정적이고 선명하게 표시됩니다. 앱이 홀로그램의 깊이를 올바르게 설정하지 않으면 이 업데이트 후 이 증상이 더 두드러지게 됩니다.
...
- en-CA **Select** 및 en-AU에 대한 초기 설정 중에 음성 명령 선택 기능이 수정되었습니다.
